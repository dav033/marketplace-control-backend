...
import { CATEGORY_CODES_TEXT, CURATION_HEADERS, categoryBoundary, isCorporateEmail, parseCurationTsv, parseMultiPlatformReputation, summarizeContactChannels, validateCurationBatch, type CurationThreshold } from './curation';
...
export async function curateProviders(input: { city: string; category: string; instructions?: string; targetCount?: number; remainingCount?: number; scanAttempt?: number; jobId?: string; runId?: string; minRating?: number; minReviews?: number; onPhase?: CurationPhaseReporter }): Promise<GeminiCurationResult> {
  ...
  // Frontera con la categoría vecina (hoy, Repostería frente a Comida y Bebida). Va en los tres
  // prompts: descubrimiento, verificación en inglés y subagentes.
  ...
  const discoveryInstruction = isBroadFoodDiscovery
    ? `Esta es una búsqueda amplia, escaneo ${scanAttempt}. Recorre de forma sistemática restaurantes, catering, banquetes, brunch y alimentación empresarial. No confundas "candidato listo" con "prospecto descubierto": los prospectos con contacto y servicio confirmado deben salir aunque requieran revisión de reputación. No inventes datos ni fuerces una cantidad; devuelve todos los negocios reales que puedas sostener con fuentes vivas, hasta 20 en este escaneo.`
...
Investiga en la web proveedores reales de la ciudad "${city}" para la categoría "${category}".${boundaryBlock}${harvestBlock}
...
Encontrar un WhatsApp no es excusa para dejar de buscar correo: son dos búsquedas independientes y el candidato puede tener ambos. Si el negocio tiene sitio web propio (no solo redes sociales), revisa explícitamente /contacto y el pie de página buscando un correo ANTES de decidir el canal, incluso si ya tienes un número de WhatsApp verificado. Solo concluye que no hay correo corporativo después de esa revisión explícita; no te conformes con el primer contacto que encuentres.
...
Los IDs deben tener formato ABC-CC-###, donde CC es el código: ${CATEGORY_CODES_TEXT}.
...
TASK: Research REAL, currently operating businesses in the city "${city}" for the category "${category}", using your built-in web_search tool.${boundaryBlock}
...
Finding a WhatsApp number is not an excuse to stop looking for an email: they are two independent searches, and a candidate can have both. If the business has its own website (not just social media), explicitly check /contacto and the page footer for an email BEFORE deciding the channel, even if you already have a verified WhatsApp number. Only conclude there is no corporate email after that explicit check; do not settle for the first contact method you find.
...
Each ID must follow the pattern LLL-CC-###, where: LLL is a 3-letter code YOU derive from the business name (uppercase letters, e.g. "HWB" for "Hotel Windsor Barranquilla" or "AMC" for "Amalfi Cucina") — never the literal text "ABC"; CC is the fixed 2-digit category code: ${CATEGORY_CODES_TEXT}; and ### is a 3-digit sequence starting at 001 for this response, incrementing per row (001, 002, 003, ...). Example for a Comida y Bebida candidate: "AMC-02-001", not "ABC-02-001".
...
Usa únicamente WebSearch. Haz una exploración amplia por restaurantes para eventos, catering, banquetes, brunch, coffee break y alimentación empresarial. Busca nombres reales, sitios oficiales, fichas directas y datos de contacto que aparezcan públicamente. No abras páginas con WebFetch, no inventes teléfonos o correos y no uses resultados genéricos como candidatos.
...
Ciudad: ${city}. Categoría: ${category}.${boundaryBlock}
...
El ID debe tener el formato ABC-CC-###, donde ABC son tres letras y CC es el código de la categoría: ${CATEGORY_CODES_TEXT}. La Fecha Verificación es la fecha de hoy en formato AAAA-MM-DD (ej. "2026-09-19"); nunca DD/MM/AAAA.
