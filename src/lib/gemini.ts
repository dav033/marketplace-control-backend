...
import { CURATION_HEADERS, isCorporateEmail, parseCurationTsv, summarizeContactChannels, validateCurationBatch } from './curation';
...
export function fallbackReputation(payload: Record<string, unknown>): { rating: string; reviews: string; platform: string } | undefined {
  ...
  if (!/^[0-5]\.\d$/.test(rawRating)) return undefined;
  ...
  return { rating: rawRating, reviews: rawReviews, platform };
}
...
export async function curateProviders(input: { city: string; category: string; instructions?: string; targetCount?: number; scanAttempt?: number; jobId?: string; runId?: string; onPhase?: CurationPhaseReporter }): Promise<GeminiCurationResult> {
  ...
  const provider: 'gemini' | 'claude-code' = configuredProvider === 'claude-code' ? 'claude-code' : 'gemini';
  const apiKey = env('GEMINI_API_KEY');
  if (provider === 'gemini' && !apiKey) throw new Error('GEMINI_NOT_CONFIGURED');
  const model = provider === 'claude-code' ? 'sonnet' : env('GEMINI_AGENT_MODEL') || 'gemini-3.8-flash';
...
Cruza la evidencia: confirma identidad y servicio en una fuente oficial o perfil directo, y contrasta reputación, calificación y reseñas en la plataforma indicada. La fecha de verificación debe ser la fecha actual de la investigación, no una fecha antigua de la página.
Para un candidato listo cumple: calificación mínima 4.5; Tipo 1 (lugares/restaurantes) requiere mínimo 50 reseñas; Tipo 2 (servicios por encargo) requiere mínimo 15 reseñas. Los prospectos en revisión pueden quedar por debajo o usar "Sin dato", pero deben tener identidad, servicio, fuente viva y contacto comprobados.
Nivel A corresponde a 50 o más reseñas. Nivel B se usa para Tipo 2 con 15 a 49 reseñas y también para cualquier prospecto en revisión cuya reputación no pudo confirmarse.
La reputación visible es obligatoria cuando existe: si el negocio tiene ficha de Google, Tripadvisor, Booking, Matrimonios.com.co o Bodas.com.co, busca y registra la calificación exacta y la cantidad de reseñas junto con la plataforma. Solo usa "Sin dato" en calificación y reseñas cuando realmente no haya reputación pública localizable, y explícalo en la justificación. No inventes ni aproximes estos valores.
...
La justificación debe incluir la calificación, cantidad de reseñas y plataforma cuando existan; además evidencia publicada de servicios para eventos, actividad publicada dentro de los últimos 12 meses y, para un prospecto en revisión, una frase clara con el dato que falta o el umbral que no cumple.
...
Devuelve un objeto JSON válido con exactamente dos campos: "tsv" y "research_summary". En "tsv" incluye la línea de encabezados exacta ${JSON.stringify(CURATION_HEADERS.join('\t'))}, seguida de una fila por candidato con las 18 columnas separadas por tabulaciones. Dentro del JSON escapa siempre los saltos de línea como \\n y las tabulaciones como \\t; no pongas saltos de línea literales dentro del valor de "tsv". No uses tablas Markdown; no uses barras verticales dentro de las celdas. No incluyas explicaciones fuera del JSON solicitado.`;

  input.onPhase?.('researching', provider === 'claude-code' ? 'Conectando con Claude Code y WebSearch.' : 'Conectando con Google Search y contexto de URLs.', { current: 0, total: 4, label: 'Buscando candidatos' });
  let response: Response | undefined;
  let outputText: string | undefined;
  let discoveredCandidates: CurationDiscoveredCandidate[] = [];
  if (provider === 'claude-code') {
    if (isBroadFoodDiscovery) {
...
Cuando los resultados de búsqueda muestren la reputación del negocio, cópiala tal cual: "rating" con un decimal (por ejemplo "4.7"), "reviews" con el número entero de reseñas (por ejemplo "128") y "platform" con la fuente exacta ("Google", "TripAdvisor", "Booking", "Matrimonios.com.co", "Bodas.com.co" o "Facebook"). Es un dato prioritario: búscalo activamente. Si no aparece publicado, usa "Sin dato" en los tres; jamás estimes, redondees ni inventes una calificación o un número de reseñas.
...
La reputación visible es obligatoria cuando existe: si encuentras calificación y número de reseñas en la página o en su ficha, cópialos exactos junto con la plataforma ("Google", "TripAdvisor", "Booking", "Matrimonios.com.co", "Bodas.com.co" o "Facebook") y repítelos en la justificación. Si no están publicados, usa "Sin dato" en los tres y di en la justificación qué falta por confirmar. Nunca estimes ni inventes esos valores.
...
El ID debe tener el formato ABC-CC-###, donde ABC son tres letras y CC es el código de la categoría: Lugar 01, Comida y Bebida 02, Música 03, Servicios Especializados 04, Entretenimiento 05, Decoración temática 06, Fotografía y Video 07, Invitación digital 08, Menaje y mantelería 09, Carpas y mobiliario 10. La Fecha Verificación es la fecha de hoy en formato AAAA-MM-DD.
Devuelve exactamente un objeto JSON con "tsv" y "research_summary". En "tsv" usa el encabezado exacto ${JSON.stringify(CURATION_HEADERS.join('\t'))}, seguido de filas con 18 columnas separadas por tabulaciones. Usa "Sin dato" cuando no exista evidencia pública y explica el déficit en la justificación. No uses Markdown ni texto fuera del JSON.`;
      ...
      const singleRun = await runClaudeCode({
        prompt,
        onPhase: input.onPhase,
        maxTurns,
  ...
  if (!outputText) throw new Error('CLAUDE_CODE_EMPTY_RESPONSE');
  ...
  if (!parsed && !fallbackTsv) {
    console.error('Claude Code returned invalid JSON', outputText.slice(-800));
    throw new Error(provider === 'claude-code' ? 'CLAUDE_CODE_INVALID_JSON' : 'GEMINI_INVALID_JSON');
  }
  const rawTsv = typeof parsed?.tsv === 'string' && parsed.tsv.trim() ? parsed.tsv : fallbackTsv;
  if (!rawTsv) throw new Error(provider === 'claude-code' ? 'CLAUDE_CODE_NO_ROWS' : 'GEMINI_NO_ROWS');
  const baseTsv = limitCurationTsvRows(normalizeKnownSourceUrls(rawTsv));
  const tsv = appendFallbackCandidates(baseTsv, discoveredCandidates, city, category, runContext);
