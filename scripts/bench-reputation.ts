...
// Sin `expected`: no hay cifra de referencia, solo se muestra lo obtenido.
...
const results = await Promise.all(cases.map(async item => {
  ...
  else if (!item.expected) verdict = got ? 'inventado' : 'sin_dato_ok';
  ...
  return { id: item.id, name: item.name, expected: item.expected, got, verdict, error: answer.error, raw: answer.raw, attempts: answer.attempts, seconds: Math.round((Date.now() - t0) / 1000) };
}));

for (const r of results) {
  const exp = r.expected === undefined ? '?' : r.expected ? `${r.expected.rating}/${r.expected.reviews}` : 'sin reseñas';
  const got = r.got ? `${r.got.rating}/${r.got.reviews} ${r.got.platform} (${r.got.matchedBy})` : `Sin dato${r.error ? ` [${r.error}]` : ''}`;
  console.log(`${r.verdict.padEnd(14)} ${r.id}  ${r.name.padEnd(34)} esperado ${exp.padEnd(12)} obtenido ${got}  ${r.attempts}x ${r.seconds}s`);
}
